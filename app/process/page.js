...
export default function ProcessPage() {
  ...
  return (
    <div className="min-h-screen mt-20 bg-gradient-to-br from-[#0F172A] to-[#1E293B] text-white py-16 relative overflow-hidden">
      {/* Advanced Animated Background Effects */}
      <div className="absolute inset-0">
        <motion.div
          className="absolute -top-20 -left-20 w-96 h-96 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full opacity-20 filter blur-3xl"
          animate={{ rotate: 360 }}
          transition={{ duration: 120, repeat: Infinity, ease: "linear" }}
        />
        <motion.div
          className="absolute -bottom-20 -right-20 w-96 h-96 bg-gradient-to-br from-yellow-500 to-red-600 rounded-full opacity-20 filter blur-3xl"
          animate={{ rotate: -360 }}
          transition={{ duration: 120, repeat: Infinity, ease: "linear" }}
        />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          {steps.map((step, index) => (
            <motion.div
              key={index}
              className="flex flex-col sm:flex-row items-center gap-6 bg-[#1E293B] p-6 rounded-xl shadow-2xl border border-gray-700"
        ...
        >
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Testimonial 1 */}
            <motion.div
              className="bg-[#1E293B] p-6 rounded-xl shadow-2xl border border-gray-700"
              whileHover={{ scale: 1.05, transition: { duration: 0.3 } }}
            >
        ...
        >
          <a href="/contact">
            <span className="inline-block bg-blue-500 text-white px-8 py-4 rounded-lg text-lg font-semibold hover:bg-blue-400 transition cursor-pointer shadow-2xl">
              🚀 Let’s Work Together
            </span>
          </a>
        </motion.div>
